...
find /var/tmp/ovl/upper -type f    # nothing — the upper layer is empty
...
So the honest statement is: **the cost moved from pay-at-create to pay-per-file-written.** For the workload this is built for — dev, CI, and PR-review branches that read a lot and write a little — that's strictly better, and branches stay in the tens of megabytes. For a branch that rewrites its whole dataset, it's a wash. Block-level copy-on-write (ZFS, or a CSI driver with real snapshots) doesn't have this problem at all, because its copy-up granularity is a block rather than a file. That's a genuine advantage of those backends and I'd rather say so than round it off.
...
**A benchmark that contradicts your design is the most valuable output it can produce.** The number that mattered wasn't 61.9 seconds, it was 5.05 GiB — because "slow" invites tuning and "the writable layer is the size of the database" tells you the design isn't running. I nearly went looking for I/O tuning knobs first.
